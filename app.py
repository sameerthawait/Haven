...
if AUTH_ENABLED:
    AUTH_EXEMPT_EXACT = {
        ...
        "/api/version",
        "/login",
        # MVP prototype endpoints (UI stubs) — allow unauthenticated access for local testing
        "/api/graph/concepts",
        "/api/workflows/plan",
    ...
    AUTH_EXEMPT_PREFIXES = ["/static", "/api/graph", "/api/workflows", "/api/collab"]
    ...
    def _is_trusted_loopback(request: Request) -> bool:
        ...
        slip past LOCALHOST_BYPASS or spoof the internal-tool path. Haven's own
        ...
        async def dispatch(self, request: Request, call_next):
            ...
            try:
                ...
                if _hdr and secrets.compare_digest(_hdr, _ITT) and _is_trusted_loopback(request):
                    ...
                    # X-Haven-Owner, attribute the request to that user only
                    # if they exist. Authorization checks remain separate; this
                    # is just owner attribution for notes/calendar/etc.
                    _impersonate = (request.headers.get("X-Haven-Owner") or "").strip()
...
async def _startup_event():
    ...
    # `HAVEN_INPROCESS_POLLERS` from the email pollers.
    _tasks_inprocess = os.environ.get("HAVEN_INPROCESS_TASKS", "1").strip().lower()
    if _tasks_inprocess not in ("0", "false", "no", "off", ""):
        await task_scheduler.start()
    else:
        logger.info(
            "In-process task scheduler disabled (HAVEN_INPROCESS_TASKS=0); "
