...
let _sortMode = Storage.get('haven-session-sort') || 'active'; // default to last active
let _autoCreateInProgress = false; // guard against recursive auto-create
const _INCOGNITO_SESSIONS_KEY = 'haven-incognito-sessions'; // sessionStorage key for incognito session IDs
...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'Haven Chat';
...
const FOLDER_STATE_KEY = 'haven-folder-state';
const FOLDER_ORDER_KEY = 'haven-folder-order';
...
function _showSwipeHint(list) {
  if ('ontouchstart' in window && !localStorage.getItem('haven-swipe-hint-shown')) {
    const firstItem = list.querySelector('.session-item');
    if (firstItem) {
      localStorage.setItem('haven-swipe-hint-shown', '1');
...
export async function loadSessions() {
  try {
    ...
    const prefetched = sessionStorage.getItem('haven-prefetch-sessions');
    let fetched;
    if (prefetched) {
      sessionStorage.removeItem('haven-prefetch-sessions');
    ...
    // most-recent).
    const _isFirstLoad = !sessionStorage.getItem('haven-session-active');
    if (_isFirstLoad) {
      sessionStorage.setItem('haven-session-active', '1');
      if (!targetId) {
        try {
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to Haven kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'Haven Chat';
    }
    ...
    if (window.documentModule) {
      ...
      const meta = sessions.find(s => s.id === id);
      const shouldOpen = localStorage.getItem('haven-doc-open-' + id) === '1';
...
export function setSortMode(mode) {
  _sortMode = mode || null;
  if (mode) Storage.set('haven-session-sort', mode);
  else Storage.remove('haven-session-sort');
  renderSessionList();
}
