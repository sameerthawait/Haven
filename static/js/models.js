...
const COLLAPSE_KEY = 'haven-models-collapsed';
const FAVORITES_KEY = 'haven-model-favorites';
const USAGE_KEY = 'haven-model-usage';
const SORT_KEY = 'haven-model-sort';
...
export async function refreshModels(force = false) {
  ...
  try {
    ...
    if (!_cachedItems || _cachedItems.length === 0) {
      ...
      const welcomeSub = document.getElementById('welcome-sub');
      if (welcomeSub) welcomeSub.textContent = 'Your local sanctuary.';
      const welcomeTip = document.getElementById('welcome-tip');
      if (welcomeTip) {
