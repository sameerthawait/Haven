...
import * as Modals from './modalManager.js';
  ...
  async function _resolveComposeSendAccountId() {
    const activeAccountId = window.__havenActiveEmailAccount || null;
  ...
  const _docOpenKey = (sessionId) => 'haven-doc-open-' + sessionId;
  const _docMinimizedKey = (sessionId) => 'haven-doc-minimized-' + sessionId;
    ...
    try {
      const res = await fetch(`${API_BASE}/api/email/draft`, {
        ...
        body: JSON.stringify({
          ...
          account_id: window.__havenActiveEmailAccount || null,
        }),
      });
      const data = await res.json();
      if (data.success) {
  ...
  export function openPanel() {
    ...
    pane.innerHTML = `
      ...
      <div class="doc-editor-header" id="doc-editor-actions">
        ...
        <span id="doc-backlinks-badge" class="doc-version-badge" title="Backlinks" style="display:none;background:var(--accent,var(--red));color:var(--bg);margin-left:4px;cursor:pointer;">0 backlinks</span>
      ...
      <div id="doc-backlinks-panel" class="doc-version-panel hidden">
        <div class="doc-version-header">
          <span>Backlinks</span>
          <button id="doc-backlinks-close" class="doc-action-icon-btn" title="Close"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>
        </div>
        <div id="doc-backlinks-list" class="doc-version-list" style="padding:10px;display:flex;flex-direction:column;gap:8px;overflow-y:auto;height:calc(100% - 40px);"></div>
      </div>
    ...
    document.getElementById('doc-backlinks-badge')?.addEventListener('click', toggleBacklinks);
    document.getElementById('doc-backlinks-close')?.addEventListener('click', _closeBacklinksPanel);
    ...
    let _fontIdx = parseInt(localStorage.getItem('haven-doc-fontsize') || '0', 10);
    ...
    function _applyDocFont() {
      ...
      localStorage.setItem('haven-doc-fontsize', _fontIdx);
    }
    _applyDocFont();
  ...
  const _DOC_RECENTS_KEY = 'haven-doc-actions-recent';
  ...
  function _saveSuggestionsToStorage() {
    ...
    if (data.length) {
      localStorage.setItem('haven-suggestions-' + activeDocId, JSON.stringify(data));
    } else {
      localStorage.removeItem('haven-suggestions-' + activeDocId);
    }
  }
  ...
  function _restoreSuggestionsFromStorage(docId) {
    try {
      const raw = localStorage.getItem('haven-suggestions-' + docId);
      if (!raw) return;
  ...
  // ---- Note Backlinks view & loading ----
  async function toggleBacklinks() {
    const panel = document.getElementById('doc-backlinks-panel');
    if (!panel || !activeDocId) return;
    if (panel.classList.contains('hidden')) {
      const sidebar = document.getElementById('sidebar');
      const isMobile = window.innerWidth <= 768;
      ...
      await loadBacklinks();
      setTimeout(() => {
        _backlinksClickOutside = (e) => {
          if (!panel.contains(e.target) && e.target.id !== 'doc-backlinks-badge') {
            _closeBacklinksPanel();
          }
        };
        document.addEventListener('click', _backlinksClickOutside, true);
      }, 0);
    } else {
      _closeBacklinksPanel();
    }
  ...
  let _backlinksClickOutside = null;

  function _closeBacklinksPanel() {
    const panel = document.getElementById('doc-backlinks-panel');
    if (panel) panel.classList.add('hidden');
    if (_backlinksClickOutside) {
      document.removeEventListener('click', _backlinksClickOutside, true);
      _backlinksClickOutside = null;
    }
  }

  async function loadBacklinks() {
    if (!activeDocId) return;
    const list = document.getElementById('doc-backlinks-list');
    if (!list) return;
    list.innerHTML = '<div style="opacity:0.5;font-size:11px;">Loading backlinks…</div>';
    try {
      const res = await fetch(`${API_BASE}/api/document/${activeDocId}/backlinks`, { credentials: 'same-origin' });
      if (!res.ok) throw new Error();
      const backlinks = await res.json();
      list.innerHTML = '';
      if (!backlinks || backlinks.length === 0) {
        list.innerHTML = '<div style="opacity:0.5;font-size:11px;">No backlinks found. Link documents using [[WikiLinks]]!</div>';
        return;
      }
      backlinks.forEach(link => {
        const item = document.createElement('div');
        item.style.cssText = 'padding:8px;border:1px solid var(--border);border-radius:6px;background:var(--bg-dark);cursor:pointer;transition:border-color 0.15s;display:flex;align-items:center;justify-content:between;';
        item.innerHTML = `
          <div style="flex:1;min-width:0;">
            <div style="font-weight:500;font-size:12px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:var(--fg);">${uiModule.esc(link.title || 'Untitled')}</div>
            <div style="font-size:10px;opacity:0.5;margin-top:2px;">Last updated ${uiModule.esc(new Date(link.updated_at).toLocaleDateString())}</div>
          </div>
        `;
      ...
      list.innerHTML = '<div style="color:var(--red);font-size:11px;">Failed to load backlinks.</div>';
    }
  }

  async function updateBacklinks(docId) {
    const badge = document.getElementById('doc-backlinks-badge');
    if (!badge) return;
    try {
      const res = await fetch(`${API_BASE}/api/document/${docId}/backlinks`, { credentials: 'same-origin' });
      if (!res.ok) throw new Error();
      const backlinks = await res.json();
      if (backlinks && backlinks.length > 0) {
        badge.textContent = `${backlinks.length} backlink${backlinks.length !== 1 ? 's' : ''}`;
        badge.style.display = '';
      } else {
        badge.style.display = 'none';
      }
