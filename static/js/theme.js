...
export const THEMES = {
  haven:      { bg:'#0a0b10', fg:'#f1f5f9', panel:'#11131e', border:'#1e2235', red:'#6366f1',
                advanced: { brandColor:'#818cf8', sendBtnBg:'#6366f1', sendBtnHover:'#4f46e5',
                            userBubbleBg:'#1e2235', aiBubbleBg:'#11131e',
                            inputBg:'#0e1017', inputBorder:'#1e2235',
                            sidebarBg:'#07080c', bubbleBorder:'#1e2235',
                            accentPrimary:'#6366f1' } },
  dark:       { bg:'#090d10', fg:'#cfd8dc', panel:'#12181c', border:'#202a30', red:'#00bfa5' },
...
const DEFAULT_THEME = 'haven';
const LS_KEY = 'haven-theme';
const CUSTOM_THEMES_KEY = 'haven-custom-themes';
const REBRAND_THEME_KEY = 'haven-rebrand-theme-v1';
...
const ADV_KEYS = [
  ...
  { key: 'brandColor',         css: '--brand-color',       label: 'Haven Logo',    group: 'Sidebar' },
...
function _updateFavicon(fg) {
  ...
  if (routeShape) {
    ...
    svg = `<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 32 32'><path d='M16 3 L26 7 V15 C26 22 16 28 16 28 C16 28 6 22 6 15 V7 Z' fill='none' stroke='${fg}' stroke-width='2.5' stroke-linejoin='round'/><path d='M16 9 L21 14 L16 19 L11 14 Z' fill='${fg}' opacity='0.8'/></svg>`;
  }
...
export function initThemeUI() {
  ...
  if (exportBtnEl) {
    ...
    newExp.addEventListener('click', () => {
      ...
      a.href = url;
      a.download = 'haven_' + (obj.name || 'theme') + '.json';
      a.click();
...
function _initPerlinFlow() {
  ...
  function getBg() { return getComputedStyle(document.documentElement).getPropertyValue('--bg').trim() || '#090d10'; }
...
async function _initWithSync() {
  const localTheme = getSaved();
  if (Storage.get(REBRAND_THEME_KEY) !== '1') {
    Storage.setJSON(LS_KEY, {
      name: DEFAULT_THEME,
      colors: THEMES[DEFAULT_THEME],
      bgPattern: THEME_DEFAULT_PATTERN[DEFAULT_THEME],
      bgEffectColor: THEME_DEFAULT_EFFECT_COLOR[DEFAULT_THEME],
      bgEffectIntensity: THEME_DEFAULT_INTENSITY[DEFAULT_THEME],
      frosted: THEME_DEFAULT_FROSTED[DEFAULT_THEME] === true,
    });
    Storage.set(REBRAND_THEME_KEY, '1');
  }
