...
// ── Library state ──
// Expose active tab globally so background animations can read it
var _activeLibTab = 'documents';
...
// ---- Library animation helpers ----
  ...
  export function openLibrary(opts) {
    ...
    modal.innerHTML = `
      <div class="modal-content doclib-modal-content" style="width:min(640px, 92vw);max-height:85vh;background:var(--bg);">
        ...
        <div class="modal-body" style="display:flex;flex-direction:column;gap:10px;overflow:hidden;">
          ...
          <div id="doclib-panel-graph" data-doclib-panel="graph" class="admin-card" style="display:none;flex:1;flex-direction:column;overflow:hidden;height:100%;">
            <div style="display:flex;align-items:baseline;gap:8px;margin-bottom:2px;margin-top:10px;">
              <h2 style="margin:0;padding:0;line-height:1;">Note Graph</h2>
            </div>
            <p class="memory-desc doclib-desc" style="position:relative;top:-1px;">Visual map of connections between your documents. Use [[WikiLinks]] in documents to link them together.</p>
            <div id="doclib-graph-container" style="flex:1;position:relative;background:#0d0d0e;border:1px solid var(--border);border-radius:8px;overflow:hidden;min-height:350px;display:flex;flex-direction:column;">
              <canvas id="doclib-graph-canvas" style="flex:1;width:100%;height:100%;display:block;"></canvas>
              <div style="position:absolute;bottom:10px;right:10px;display:flex;gap:5px;">
                <button class="memory-toolbar-btn" id="doclib-graph-zoom-in" style="padding:2px 6px;">+</button>
                <button class="memory-toolbar-btn" id="doclib-graph-zoom-out" style="padding:2px 6px;">-</button>
                <button class="memory-toolbar-btn" id="doclib-graph-reset" style="padding:2px 6px;">Reset</button>
              </div>
            </div>
          </div>
    ...
    _activeLibTab = (opts && opts.tab) || 'documents';
    ...
    const _TAB_HEADERS = {
      ...
      graph: {
        label: 'Note Graph',
        svg: '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg>',
      },
      research: {
        label: 'Research',
    ...
    function _switchLibTab(tab) {
      ...
      else if (tab === 'graph') _renderLibGraph();
    }
  ...
  // ---- Note Graph physics & canvas rendering ----
  let _graphNodes = [];
  let _graphLinks = [];
  let _graphZoom = 1.0;
  let _graphPan = { x: 0, y: 0 };
  ...
  function _renderLibGraph() {
    _graphCanvas = document.getElementById('doclib-graph-canvas');
    ...
    const container = document.getElementById('doclib-graph-container');
    if (container) {
      _graphCanvas.width = container.clientWidth;
      _graphCanvas.height = container.clientHeight || 400;
    }

    fetch(API_BASE + '/api/documents/graph', { credentials: 'same-origin' })
      .then(r => r.json())
      .then(data => {
        _graphNodes = data.nodes || [];
        _graphLinks = data.links || [];

        const cx = _graphCanvas.width / 2;
        const cy = _graphCanvas.height / 2;

        _graphNodes.forEach(node => {
      ...
      .catch(err => {
        console.error("Failed to load note graph:", err);
        if (_graphCtx) {
          _graphCtx.fillStyle = '#ff4d4d';
          _graphCtx.font = '12px sans-serif';
          _graphCtx.fillText("Failed to load note graph: " + err.message, 20, 30);
          if (err.stack) {
            console.error(err.stack);
            _graphCtx.fillText(err.stack.slice(0, 80), 20, 50);
          }
        }
      });
  }

  function _runGraphSimulation() {
    if (!_graphCanvas || !_graphCtx || _activeLibTab !== 'graph') return;
    _updateGraphPhysics();
    _drawGraph();
    _graphAnimFrame = requestAnimationFrame(_runGraphSimulation);
  }

  function _updateGraphPhysics() {
    const width = _graphCanvas.width;
    const height = _graphCanvas.height;
    const cx = width / 2;
    const cy = height / 2;
    ...
    const nodeMap = {};
    _graphNodes.forEach(n => { nodeMap[n.id] = n; });
    ...
    _graphLinks.forEach(link => {
      ...
      const force = kAttract * (dist - linkRestLength);
      const fx = (dx / dist) * force;
      const fy = (dy / dist) * force;

      if (source !== _draggedNode) {
        source.vx += fx;
        source.vy += fy;
      }
      if (target !== _draggedNode) {
        target.vx -= fx;
        target.vy -= fy;
      }
    });
  ...
  function _drawGraph() {
    ...
    ctx.translate(cx + _graphPan.x, cy + _graphPan.y);
    ...
    _graphNodes.forEach(node => {
      let color = '#ea580c';
      if (node.language === 'markdown' || node.language === 'md') color = '#22c55e';
      else if (node.language === 'python') color = '#3b82f6';
      else if (node.language === 'html' || node.language === 'css') color = '#ec4899';
      else if (node.language === 'javascript' || node.language === 'typescript') color = '#eab308';
      else if (node.language === 'email') color = '#a855f7';
      ...
      if (_graphZoom > 0.75 || node === _hoveredNode) {
    ...
    if (_hoveredNode) {
      ...
      ctx.strokeStyle = 'var(--border, rgba(255,255,255,0.15))';
      ctx.lineWidth = 1;

      const text = _hoveredNode.title + " (" + (_hoveredNode.language || "text") + ")";
      ctx.font = '12px sans-serif';
      const textWidth = ctx.measureText(text).width;

      const padX = 12;
      const padY = 8;
      const w = Math.max(180, textWidth + padX * 2);
      const h = 42;
      ...
      ctx.font = '10px sans-serif';
      ctx.fillText("Double-click node to open", tx + padX, ty + padY + 16);
    }
  }
  ...
  function _initGraphInteraction() {
    ...
    newCanvas.id = 'doclib-graph-canvas';
    newCanvas.style.cssText = 'flex:1;width:100%;height:100%;display:block;';
    canvas.parentNode.replaceChild(newCanvas, canvas);
    _graphCanvas = newCanvas;
    _graphCtx = newCanvas.getContext('2d');

    const c = _graphCanvas;
    let _graphIsPanning = false;
    let _graphDragStart = { x: 0, y: 0 };
    ...
    const screenToGraph = (sx, sy) => {
      ...
      const gx = ((mx - cx - _graphPan.x) / _graphZoom) + cx;
      const gy = ((my - cy - _graphPan.y) / _graphZoom) + cy;
    ...
    c.addEventListener('mousedown', (e) => {
      ...
      if (clickedNode) {
        ...
        _graphIsPanning = true;
        _graphDragStart = { x: e.clientX - _graphPan.x, y: e.clientY - _graphPan.y };
      }
    });
    ...
    c.addEventListener('mousemove', (e) => {
      ...
      if (_draggedNode) {
        _draggedNode.x = pos.x;
        _draggedNode.y = pos.y;
      } else if (_graphIsPanning) {
        _graphPan.x = e.clientX - _graphDragStart.x;
        _graphPan.y = e.clientY - _graphDragStart.y;
      } else {
        let hoverNode = null;
        for (let i = 0; i < _graphNodes.length; i++) {
          const n = _graphNodes[i];
    ...
    c.addEventListener('wheel', (e) => {
      ...
      if (e.deltaY < 0) {
        _graphZoom = Math.min(_graphZoom * zoomFactor, 3.0);
      } else {
        _graphZoom = Math.max(_graphZoom / zoomFactor, 0.25);
      }
    }, { passive: false });

    const zoomIn = document.getElementById('doclib-graph-zoom-in');
    const zoomOut = document.getElementById('doclib-graph-zoom-out');
    const reset = document.getElementById('doclib-graph-reset');

    if (zoomIn) zoomIn.onclick = () => { _graphZoom = Math.min(_graphZoom * 1.15, 3.0); };
    if (zoomOut) zoomOut.onclick = () => { _graphZoom = Math.max(_graphZoom / 1.15, 0.25); };
    if (reset) reset.onclick = () => { _graphZoom = 1.0; _graphPan = { x: 0, y: 0 }; };
  }
