// ============================================
// Haven UI — Main Application Orchestrator
...
// App branding
window.APP_NAME = 'Haven';
window.APP_TITLE = 'Haven AI';
...
async function _refreshDefaultChat() {
  try {
    ...
    if (d && d.endpoint_url && d.model) {
      _defaultChat = d;
      try { window.__havenDefaultChat = d; } catch (_) {}
      return d;
    }
  } catch (_) {}
  return null;
...
//  document.getElementById('chat-form').addEventListener('submit', chatModule.handleChatSubmit);
  ...
  if (exportPdfBtn) {
    exportPdfBtn.addEventListener('click', (e) => {
      ...
      const sessionName = meta ? meta.name : 'Haven Chat';
  ...
  const toolGraphBtn = el('tool-graph-btn');
  if (toolGraphBtn) {
    toolGraphBtn.addEventListener('click', async () => {
      const modal = document.createElement('div');
      modal.id = 'graph-modal';
      modal.className = 'modal';
      modal.innerHTML = `
        <div class="modal-content admin-modal-content" style="width:500px;">
          <div class="modal-header" style="display:flex;justify-content:space-between;align-items:center;">
            <h3 style="margin:0;">Concept Graph</h3>
            <button type="button" class="close-btn" data-close-graph>✕</button>
          </div>
          <div style="padding:16px;min-height:200px;">
            <p style="margin:0 0 12px 0;opacity:0.7;">Visualizing semantic relationships in your documents...</p>
            <div id="graph-container" style="width:100%;height:250px;border:1px solid var(--border);border-radius:4px;padding:12px;background:var(--panel);overflow:auto;"></div>
          </div>
        </div>
      `;
      ...
      const closeBtn = modal.querySelector('[data-close-graph]');
      if (closeBtn) {
      ...
      try {
        ...
        if (container) {
          if (data.message) {
            container.innerHTML = `<p style="opacity:0.6;margin:0;">${data.message}</p>`;
          } else {
            container.innerHTML = `<pre style="margin:0;font-size:11px;overflow:auto;">${JSON.stringify(data, null, 2)}</pre>`;
          }
        }
      } catch (e) {
        const container = document.getElementById('graph-container');
        if (container) container.innerHTML = `<p style="color:var(--red);margin:0;">Error: ${e.message}</p>`;
      }
    });
  }

  // Workflows button
  const toolWorkflowsBtn = el('tool-workflows-btn');
  if (toolWorkflowsBtn) {
    toolWorkflowsBtn.addEventListener('click', async () => {
      const modal = document.createElement('div');
      modal.id = 'workflows-modal';
      modal.className = 'modal';
      modal.innerHTML = `
        <div class="modal-content admin-modal-content" style="width:600px;">
          <div class="modal-header" style="display:flex;justify-content:space-between;align-items:center;">
            <h3 style="margin:0;">Actionable Workflows</h3>
            <button type="button" class="close-btn" data-close-workflows>✕</button>
          </div>
          <div style="padding:16px;">
            <div style="margin-bottom:12px;">
              <label style="display:block;margin-bottom:6px;font-size:12px;opacity:0.8;">Describe your task:</label>
              <input type="text" id="workflow-task-input" placeholder="e.g. schedule a meeting with the team" style="width:100%;padding:8px;border:1px solid var(--border);border-radius:4px;background:var(--bg);color:var(--fg);box-sizing:border-box;" />
            </div>
            <div style="display:flex;gap:8px;margin-bottom:16px;">
              <button type="button" class="confirm-btn confirm-btn-primary" id="plan-btn" style="flex:1;">Generate Plan</button>
              <button type="button" class="confirm-btn" id="execute-btn" disabled style="flex:1;">Execute (Approve)</button>
            </div>
            <div id="workflow-result" style="min-height:150px;padding:12px;background:var(--panel);border:1px solid var(--border);border-radius:4px;overflow:auto;"><p style="opacity:0.5;margin:0;font-size:12px;">Plan will appear here...</p></div>
          </div>
        </div>
      `;
      document.body.appendChild(modal);
      ...
      // Close button handler
      const closeBtn = modal.querySelector('[data-close-workflows]');
      if (closeBtn) {
        closeBtn.addEventListener('click', () => {
          modal.remove();
        });
      ...
      const taskInput = document.getElementById('workflow-task-input');
      ...
      planBtn.addEventListener('click', async () => {
        ...
        resultDiv.innerHTML = '<p style="opacity:0.5;margin:0;">Generating plan...</p>';
        try {
          ...
          resultDiv.innerHTML = `<pre style="margin:0;font-size:11px;overflow:auto;">${JSON.stringify(currentPlan, null, 2)}</pre>`;
          executeBtn.disabled = false;
        } catch (e) {
          resultDiv.innerHTML = `<p style="color:var(--red);margin:0;font-size:12px;">Error: ${e.message}</p>`;
      ...
      executeBtn.addEventListener('click', async () => {
        executeBtn.disabled = true;
        resultDiv.innerHTML = '<p style="opacity:0.5;margin:0;">Executing workflow...</p>';
        try {
          ...
          resultDiv.innerHTML = `<pre style="margin:0;font-size:11px;overflow:auto;">${JSON.stringify(result, null, 2)}</pre>`;
        } catch (e) {
          resultDiv.innerHTML = `<p style="color:var(--red);margin:0;font-size:12px;">Error: ${e.message}</p>`;
        } finally {
          executeBtn.disabled = false;
  ...
  const toolCollabBtn = el('tool-collab-btn');
  if (toolCollabBtn) {
    toolCollabBtn.addEventListener('click', async () => {
      const modal = document.createElement('div');
      ...
      modal.innerHTML = `
        <div class="modal-content admin-modal-content" style="width:500px;">
          <div class="modal-header" style="display:flex;justify-content:space-between;align-items:center;">
            <h3 style="margin:0;">Start Collaboration</h3>
            <button type="button" class="close-btn" data-close-collab>✕</button>
          </div>
          <div style="padding:16px;">
            <button type="button" class="confirm-btn confirm-btn-primary" id="create-session-btn" style="width:100%;padding:8px;margin-bottom:12px;">Create Session</button>
            <div id="collab-result" style="min-height:150px;padding:12px;background:var(--panel);border:1px solid var(--border);border-radius:4px;overflow:auto;font-size:11px;"><p style="opacity:0.5;margin:0;">Click to create a new session...</p></div>
          </div>
        </div>
      `;
      document.body.appendChild(modal);
      ...
      // Close button handler
      const closeBtn = modal.querySelector('[data-close-collab]');
      if (closeBtn) {
        closeBtn.addEventListener('click', () => {
          modal.remove();
        });
      ...
      createBtn.addEventListener('click', async () => {
        ...
        try {
          ...
          resultDiv.innerHTML = `
            <div style="margin-bottom:12px;">
              <p style="opacity:0.7;margin:0 0 6px 0;">Session Created:</p>
              <pre style="margin:0;font-size:10px;overflow:auto;background:var(--bg);padding:8px;border-radius:3px;">${JSON.stringify(data, null, 2)}</pre>
              <p style="opacity:0.6;margin:8px 0 0 0;font-size:10px;">Share the URL with collaborators:</p>
              <code style="background:var(--bg);padding:4px 6px;border-radius:3px;word-break:break-all;font-size:9px;">http://localhost:7000${data.url}</code>
            </div>
          `;
        } catch (e) {
          resultDiv.innerHTML = `<p style="color:var(--red);margin:0;">Error: ${e.message}</p>`;
        } finally {
  ...
  if (_opener) window._havenRouteOpener = _opener;
  ...
  if (modelSortBtn && modelSortDropdown) {
    ...
    modelSortDropdown.querySelectorAll('.sort-option').forEach(opt => {
      opt.addEventListener('click', () => {
        const mode = opt.dataset.sort;
        Storage.set('haven-model-sort', mode);
  ...
  const _prefetchedFeatures = sessionStorage.getItem('haven-prefetch-features');
  sessionStorage.removeItem('haven-prefetch-features');
  ...
  const _prefetchedSettings = sessionStorage.getItem('haven-prefetch-settings');
  sessionStorage.removeItem('haven-prefetch-settings');
...
// Voice recording is handled by the dual-purpose send/mic button (see below)
  ...
  const SPLASH_COUNT_KEY = 'haven-tool-splash-counts';
  const SPLASH_MAX = 2;
  const _toolSplashes = {
  ...
  (function initModelPickerResponsive() {
    ...
    function checkPickerOverflow() {
      ...
      if (textarea) {
        textarea.setAttribute('placeholder', w < PLACEHOLDER_HIDE_WIDTH ? '' : 'Message Haven...');
      }
  ...
  const UI_VIS_KEY = 'haven-ui-visibility';
  ...
  (function migrateOldToolbarVis() {
    const OLD_KEY = 'haven-toolbar-visibility';
    try {
...
// ============================================
function startHavenApp() {
  if (window.__havenAppStarted) return;
  window.__havenAppStarted = true;
  // Set CSS variables
  ...
  if (documentModule) {
    ...
    if (_curSession && localStorage.getItem('haven-doc-open-' + _curSession) === '1') {
  ...
  window._havenBtnIcons = { send: _sendIcon, mic: _micIcon, stop: _stopIcon, newChat: _newChatIcon };
  ...
  if (sessionModule) {
    ...
    sessionModule.loadSessions()
      ...
      .finally(() => {
        ...
        if (window._havenRouteOpener) {
          try { window._havenRouteOpener(); } catch (_) {}
          window._havenRouteOpener = null;
        }
      });
  } else {
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', startHavenApp, { once: true });
} else {
  startHavenApp();
}
