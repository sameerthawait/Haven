                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in Haven (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in Haven" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "haven_kind" if "haven_kind" in cols else "'scheduled' AS haven_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                haven_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-Haven-Origin"] = "haven-ui"
                outer["X-Haven-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", haven_kind or "scheduled")[:64]
                outer["X-Haven-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `haven-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `HAVEN_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `haven-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("HAVEN_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `HAVEN_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (HAVEN_INPROCESS_POLLERS=0); "
            "drive `haven-mail poll-scheduled` externally."
        )
        return
    import asyncio
