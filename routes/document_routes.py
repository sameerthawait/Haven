...
def setup_document_routes(session_manager, upload_handler=None) -> APIRouter:
    ...
    @router.get("/api/documents/graph")
    async def get_documents_graph(request: Request) -> Dict[str, Any]:
        """Fetch nodes (documents) and links (references parsed from wiki-style [[WikiLinks]])
        representing connections between documents for the current user.
        """
        user = get_current_user(request)
        db = SessionLocal()
        try:
            from sqlalchemy import or_
            _arch_cond = or_(Document.archived == False, Document.archived.is_(None))
            q = db.query(Document).filter(Document.is_active == True).filter(_arch_cond)
            if user:
                q = q.filter(Document.owner == user)
            ...
            for doc in docs:
                if doc.title:
                    title_to_id[doc.title.strip().lower()] = doc.id
                id_set.add(doc.id)
                nodes.append({
                    "id": doc.id,
            ...
            logger.error(f"Failed to compile note graph: {e}")
            raise HTTPException(500, f"Failed to compile note graph: {e}")
        finally:
            db.close()
    ...
    async def prepare_signed_reply(doc_id: str, request: Request):
        ...
        # as email_routes (HAVEN_MAIL_ATTACHMENTS_DIR).
        from pathlib import Path as _Path
        import os as _os
        _DATA_DIR = _Path(__file__).resolve().parent.parent / "data"
        _BASE = _os.environ.get("HAVEN_MAIL_ATTACHMENTS_DIR", str(_DATA_DIR / "mail-attachments"))
    ...
    # ---- GET /api/document/{doc_id}/backlinks ----
    @router.get("/api/document/{doc_id}/backlinks")
    async def get_document_backlinks(request: Request, doc_id: str) -> List[Dict[str, Any]]:
        """Find other documents that reference this document (by ID or Title) using [[WikiLinks]]."""
        user = get_current_user(request)
        db = SessionLocal()
        try:
            doc = db.query(Document).filter(Document.id == doc_id).first()
            ...
            from sqlalchemy import or_
            _arch_cond = or_(Document.archived == False, Document.archived.is_(None))
            q = db.query(Document).filter(Document.is_active == True).filter(_arch_cond).filter(Document.id != doc_id)
            if user:
                q = q.filter(Document.owner == user)
            ...
            backlinks = []
            doc_title_lower = doc.title.strip().lower() if doc.title else None
            
            for other in other_docs:
                content = other.current_content or ""
                matches = wiki_pattern.findall(content)
                linked = False
                for target_ref in matches:
                    target_ref = target_ref.strip().lower()
                    if target_ref == doc_id.lower() or (doc_title_lower and target_ref == doc_title_lower):
                        linked = True
                        break
                if linked:
                    backlinks.append({
            ...
            logger.error(f"Failed to fetch backlinks: {e}")
            raise HTTPException(500, f"Failed to fetch backlinks: {e}")
        finally:
            db.close()
